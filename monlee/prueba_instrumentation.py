...
class FrameManu(Frame):
        ...
        def create_widgets(self):
            ...
            self.btn1 = Button(self, text='MEASUREMENTS', fg="black", bg="gray", command= self.window_measurements)
            ...
            self.btn5 = Button(self, text='CONTROL LIGHT', fg="black", bg="gray", command= self.window_light)
            self.btn5.place(relx=0.74, rely=0.1, relwidth=0.15, relheight=0.08)
        ...
        def window_configuration(self):
            ...
            #pH
            textoPh = Label(self,ventana_configuration, text = "pH")
        ...
        def fn_light_red_Control(self):
            cad = 'F720DF'
            self.arduino.write(cad.encode('ascii'))
            print(cad)

        def fn_light_green_Control(self):
            cad = 'F7A05F'
            print(cad)

        def fn_light_blue_Control(self):
            cad = 'F7609F'
            print(cad)

        def window_light(self):
            ventana_light = Toplevel()
            ventana_light.title("LIGHT CONTROL")
            ventana_light.geometry("500x400")
            boton_red = Button(ventana_light, text='RED', fg="black", bg="red", command=self.fn_light_red_Control)
            boton_red.place(relx=0.10, rely=0.1, relwidth=0.15, relheight=0.08)
            boton_green = Button(ventana_light, text='GREEN', fg="black", bg="green", command=self.fn_light_green_Control)
            boton_green.place(relx=0.1, rely=0.2, relwidth=0.15, relheight=0.08)
            boton_blue = Button(ventana_light, text='BLUE', fg="black", bg="blue", command=self.fn_light_blue_Control)
